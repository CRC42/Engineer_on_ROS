...
bool WitRos::init(NodeHandle &nh) {
  ...
  /************** 外参矩阵(lidar -> imu)****************/
  nh.getParam("imu_ext_R",imu_ext_R_data);
  nh.getParam("imu_ext_t",imu_ext_t_data);
  
  if (( imu_ext_R_data.size() != 9 ) ||( imu_ext_t_data.size() != 3 ) )
  {
    std::cout << "Load camera_ext parameter fail!!!, please check!!!" << std::endl;
  }
  m_imu_ext_R_data= Eigen::Map< Eigen::Matrix< double, 3, 3, Eigen::RowMajor > >(imu_ext_R_data.data());
  m_imu_ext_t_data = Eigen::Map< Eigen::Matrix< double, 3, 1 > >(imu_ext_t_data.data());
...
void WitRos::processStreamData() {
  if (ok()) {
    ...
    */
    ...
    Eigen::Vector3d gry(data.w[0],data.w[1],data.w[2]);
    gry = m_imu_ext_R_data * gry;
    imu_msg.angular_velocity.x = gry.x();
    imu_msg.angular_velocity.y = gry.y();
    imu_msg.angular_velocity.z = gry.z();
    ...
    */
    ...
    // 平移部分在loam/include/common_lib.h修改
  
    Eigen::Vector3d acc(data.a[0], data.a[1], data.a[2]);
    acc = m_imu_ext_R_data * acc;
    ...
    Eigen::Quaterniond imu_q(imu_msg.orientation.w, imu_msg.orientation.x, imu_msg.orientation.y, imu_msg.orientation.z);
    Eigen::Quaterniond m_imu_ext_R_Q_data = Eigen::Quaterniond(m_imu_ext_R_data).inverse();
    imu_q = imu_q * m_imu_ext_R_Q_data;
    imu_q.normalize();
